...
#model3 <- glm(peranyipv ~ anyhardship + nb_adults + size_hardship + age + sex_orient2 + schoolemploy + blk + alcoholfreq + inrelationship + anychild + nb_children + nb_seniors + HIV + cov_anxious + cov_worried + cov_upset + cov_nocope, data = anyipvcase, family = binomial)
...
#### Crude Table 1
```{r}
vars1<-c("age", "sex_orient2", "schoolemploy","inc_cat", "blk", "inrelationship" ,"anychild", "nb_children", "nb_adults", "nb_seniors", "HIV" , "alcoholfreq", "cov_anxious", "cov_worried", "cov_upset", "cov_nocope")

table1crude <- CreateTableOne(vars = vars1,data = anyipvcase, strata = "anyhardship", includeNA = F, smd=T, test=F)
...
# export a table
write.csv(table1crude_print,"C:/Users/16174/Documents/SFU/Rishav_Campion/ayazazi_econhardship_IPV_table1crude_06dec2023a.csv")
```
##### interpretations
...
table1wt.1 <- svyCreateTableOne(vars = vars1, data = weighted_table1_data)

xtabs(~anyipvcase$peranyipv+anyipvcase$anyhardship)
, weights(anyipvcase$mw_new)

table1wt_print <- print(table1wt, test=FALSE, smd=TRUE) 
table1wt_print

# export a table
  write.csv(table1wt_print,"C:/Users/16174/Documents/SFU/Rishav_Campion/ayazazi_econhardship_IPV_table1wt_06dec2023a.csv")
```
##### interpretations
...
```{r}
ggplot(combined_df, aes(x = Category, y = Estimate, color = Analysis)) +
  geom_point(size = 3, position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.2, position = position_dodge(width = 0.3)) +
  geom_text(aes(label = round(Estimate, 2)), vjust = -0.5, position = position_dodge(width = 0.3), size = 3) +
  labs(
    title = "IPV Perpetration Estimates and Confidence Intervals",
    x = "IPV Type",
    y = "Estimate"
  ) +
  theme_minimal() +
  scale_color_manual(
    values = c("#0000FF", "#FFA500"),
    labels = c("Covariate-Adjusted", "Sensitivity Analysis")
  ) +
